...
export default async function OwnersPage() {
  const owners = await getInvestors();
    ...
    <div className="flex-1 flex justify-center px-6">
      <div className="w-full max-w-xl border-b border-border bg-surface">
        <div className="px-5 md:px-7 pt-5 md:pt-6 pb-4 md:pb-5">
          {owners.length === 0 ? (
            <div className="h-6 w-full bg-border" />
          ) : (
            <div
        ...
        {owners.length === 0 ? (
          <div className="px-5 md:px-7 py-8 text-center text-muted font-mono text-xs uppercase tracking-widest">
            No investors yet
          </div>
        ) : (
